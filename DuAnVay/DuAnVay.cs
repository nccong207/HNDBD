...
namespace DuAnVay
{
    public class DuAnVay:ICControl
    {
        ...
        void Master_ColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
            ...
            if (e.Column.ColumnName.ToUpper() == "NGAYLAP")
            {
...
//                }
        ...
        bool CheckNguoiVay(string SoCMND, string SoHK, DateTime NgayLap)
        {
            using (DataTable dt = db.GetDataTable(string.Format(@"EXEC sp_CheckNguoiVay '{0}','{1}','{2}'", SoCMND, SoHK, NgayLap)))
            {
                ...
                object oVay = dt.Compute("sum(DangVay)", "");
                object oTuoi = dt.Rows[0]["Tuoi"];
                if ((oVay != DBNull.Value && Convert.ToDecimal(oVay) > 30000000)  //vay tren 30 trieu
                    || (oTuoi != DBNull.Value && (Convert.ToInt32(oTuoi) < 18 || Convert.ToInt32(oTuoi) > 75)))     //18 den 75
                {
                    XtraMessageBox.Show("Điều kiện vay chưa hợp lệ, vui lòng kiểm tra lại các yếu tố sau:\n- Nam nữ trong độ tuổi lao động (18 - 70)\n- Hộ dân đang tham gia 02 dự án vay vốn khác\n- Tổng vốn đang vay trên 30 triệu", Config.GetValue("PackageName").ToString());
                    return false;
                }
                if (dt.Select("DangVay > 0").Length >= 1)                 //dang vay o du an khac (ko xử lý trường hợp sửa dữ liệu và chọn lại chính hộ đó)
                {
                    XtraMessageBox.Show("Hộ dân đang tham gia dự án vay vốn khác", Config.GetValue("PackageName").ToString());
